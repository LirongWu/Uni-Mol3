...
from .transformer import TransformerEncoderWithPair, TransformerDecoder
...
@register_model("unimol3")
class UniMol3Model(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def forward(
        ...
        **kwargs
        ...
        def one_block(x, pos, return_x=False):
            ...
            indices = None
            if tokenization:
                x_logits, indices = self.classification_heads['quantizer'](x[:, 1:, :])
                x = self.classification_heads['decoder'](x_logits, attn_mask=attn_mask[:, :, 1:, 1:])

                node_output = self.classification_heads['move_pred'](
                    x,
                    self.classification_heads['se3_kernel'](dist.detach(), pair_type),
                    attn_mask[:, :, 1:, 1:],
                    delta_pos.detach(),
                )
            else:
            ...
            if return_x:
                return x_logits, x, pair, pos + node_output, indices
            else:
                return pos + node_output

        x_logits, x, pair, pos, indices = one_block(x, pos, return_x=True)
        ...
        if not features_only:
            if self.args.masked_token_loss > 0:
                logits = self.lm_head(x, encoder_masked_tokens)
    ...
    def register_tokenization_head(
        self, inner_dim=None, **kwargs
    ):
        """Register a tokenization head."""
        if self.args.trainmode == 1:
            enc_layers = 5
        else:
            enc_layers = 3
        if self.args.trainmode == 2:
            levels_list = [7, 5, 5, 5, 5]
        else:
            levels_list = [8, 5, 5, 5]

        self.classification_heads['decoder'] = TransformerDecoder(
            num_encoder_layers=enc_layers,
            embedding_dim=self.args.encoder_embed_dim,
            ffn_embedding_dim=self.args.encoder_ffn_embed_dim,
            num_attention_heads=self.args.encoder_attention_heads,
        ...
        self.classification_heads['quantizer'] = FSQ(dim=self.args.encoder_embed_dim, levels=levels_list)

        self.classification_heads['move_pred'] = MovementPredictionHead(
            self.args.encoder_embed_dim, self.args.pair_embed_dim, self.args.encoder_attention_heads
        )

        self.classification_heads['se3_kernel'] = SE3InvariantKernel(
...
@register_model_architecture("unimol3", "unimol3")
...
@register_model_architecture("unimol3", "unimol3_base")
def unimol_base_architecture(args):
    base_architecture(args)

@register_model_architecture("unimol3", "unimol3_84M")
def unimol_base_architecture(args):
    base_architecture(args)


@register_model_architecture("unimol3", "unimol3_164M")
def unimol_base_architecture(args):
...
@register_model_architecture("unimol3", "unimol3_310M")
def unimol_base_architecture(args):
...
@register_model_architecture("unimol3", "unimol3_570M")
def unimol_base_architecture(args):
    args.encoder_layers = getattr(args, "encoder_layers", 32)
...
@register_model_architecture("unimol3", "unimol3_1100M")
def unimol_base_architecture(args):
